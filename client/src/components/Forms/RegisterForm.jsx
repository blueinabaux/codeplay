...
const teamExistUrl = import.meta.env.VITE_TEAM_EXIST_URL;
const register = import.meta.env.VITE_REG_URL;
const utrExist = import.meta.env.VITE_UTR_EXIST_URL;
...
const RegisterForm = () => {
  ...
  const paymentArray = [{ id: 1, pay: "QR" }];
  ...
  const reg = async () => {
    ...
    if (
      ...
      const isMobileValid =
        !isNaN(mobile) && mobile.length > 7 && mobile.length < 15;

      if (isEmailValid && isMobileValid) {
        // setEmailError('Valid Email :)')
        try {
          await axios.post(`${register}`,uData);
            setAlert({
      ...
      setAlert({
        message: "Invalid Input or Team Name already exists!",
        severity: "warning",
        visible: true,
      });
    }
  ...
  const checkUTR = async () => {
    ...
    if (!utr) {
      setAlert({
        message: "Please enter correct UTR",
        severity: "warning",
        visible: true,
      });
      return false;
    ...
    try {
      ...
      setAlert({
        message: "Error checking Transaction ID",
        severity: "error",
        visible: true,
      });
      return false;
  ...
  const handleRegister = async () => {
    ...
    if (!teamExists) {
      const utrExists = await checkUTR();
      if (!utrExists) {
        reg();
      }
    }
  ...
  return (
    <>
      <div className="alert-main h-auto w-auto fixed top-[3%] z-[20000]">
        {alert.visible && (
      ...
      {loading && (
        <div className="loader h-[100%] w-[100%] absolute bg-[#1818186f] backdrop-blur-[2px] flex justify-center items-center z-[20000]">
          <Box sx={{ display: "flex" }}>
            <CircularProgress />
          </Box>
        </div>
      )}
      <div className="reg-form-main max-sm:h-auto h-[80vh] mt-[10vh] max-sm:w-[95vw] w-[80vw] bg-zinc-800 max-sm:flex-col flex justify-center items-center rounded-[1vw]">
        <QrModal />
        ...
        <div className="form-right h-full max-sm:h-[50vh] w-[50%] max-sm:w-[100%] bg--900 flex flex-col justify-start items-center bg--700 ">
          <div className="top-reg max-sm:hidden h-[15%] w-full flex justify-start items-center bg--500 px-[2vw]"></div>

          <div className="fomr-content h-[70%] w-full flex flex-col justify-evenly items-center">
            <input
              onChange={addChange}
              value={uData.email}
              className=" text-white max-sm:w-[90%] max-sm:px-[5vw] outline-none w-[80%] rounded-full px-[2vw] py-[1.5vh] bg-zinc-700 "
              id="email"
              name="email"
              type="email"
              placeholder="Email"
          ...
          <button
            ...
            className=" w-[40%] px-[2vw] py-[1vh] max-sm:mt-[3vh] bg-white border-[1px] border-solid border-white hover:bg-transparent rounded-full max-sm:w-[70%] max-sm:py-[1.5vh]  hover:text-white font-bold"
          >
            REGISTER
          </button>
        </div>
